...
def handleMessage(message):
            ...
            if entity == "BVN":
                ...
                if "Successful" in str(responseMessage["responseMessage"]):
                    ...
                    try:
                        ...
                        print("exception :( ", ex)
            elif json.loads(json.dumps(response, indent=2))['entities'][0]['entity'] == "option":
                send_url = 'http://freegeoip.net/json'
                r = requests.get(send_url)
                j = json.loads(r.text)
                location["lat"] = j['latitude']
                location["lng"] = j['longitude']
                print location
                if len(j) != 0:
                    sql = "SELECT * FROM LOCATION ORDER BY ID DESC fetch first 1 row only"
                    stmt = ibm_db.exec_immediate(conn, sql)
                    location["id"] = 1
                    while ibm_db.fetch_row(stmt) != False:
                        location["id"] = 1 + int(ibm_db.result(stmt, "ID"))
                    sql = "INSERT INTO LOCATION (ID, LATITUDE, LONGITUDE, TOC) VALUES (?, ?, ?, ?)"
                    stmt = ibm_db.prepare(conn, sql)
                    param = location["id"], location["lat"], location["lng"], datetime.datetime.utcnow(),
                ...
                if len(json.loads(json.dumps(response, indent=2))['output']['text']) != 0:
                    try:
                        ...
                        print("exception :( ", ex)
            elif len(json.loads(json.dumps(response, indent=2))['entities'][0]['metadata']) != 0:
    ...
    emit('my response', {'data': bot_response, 'intent': intent, 'entity': entity, 'user': user, 'form': form, 'locate': option, 'count': session['receive_count']})

    conversationLog = {'data': bot_response, 'human': from_human_message, 'intent': intent, 'entity': entity, 'user': user, 'form': form, 'count': session['receive_count']}
    if conversationLog["data"] != "":
        sql = "SELECT * FROM CONVERSATIONLOG ORDER BY ID DESC fetch first 1 row only"
        stmt = ibm_db.exec_immediate(conn, sql)
        conversationLog["id"] = 1
        while ibm_db.fetch_row(stmt) != False:
            conversationLog["id"] = 1 + int(ibm_db.result(stmt, "ID"))
        sql = "INSERT INTO CONVERSATIONLOG (ID, BOTMESSAGE, HUMANMESSAGE, INTENT, ENTITY, TOC) VALUES (?, ?, ?, ?, ?, ?)"
        stmt = ibm_db.prepare(conn, sql)
        param = conversationLog["id"], conversationLog["data"], conversationLog["human"], conversationLog["intent"], conversationLog["entity"], datetime.datetime.utcnow(),
        ibm_db.execute(stmt, param)
